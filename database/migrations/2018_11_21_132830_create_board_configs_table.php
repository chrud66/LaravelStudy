...
class CreateBoardConfigsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('board_configs', function (Blueprint $table) {
            ...
            $table->unsignedTinyInteger('use_comment_reply')->comment('대댓글 사용 여부')->default(0);
            ...
            $table->string('permission_reply')->comment('게시판 댓글 달기 권한 설정')->nullable();
            $table->string('permission_manage')->comment('게시판 관리 권한 설정')->nullable();
            $table->unsignedTinyInteger('use_vote_up')->comment('추천 사용 여부 [ 0 : 미사용, 1 : 사용 ]')->default(0);
            $table->unsignedTinyInteger('use_vote_down')->comment('비추천 사용 여부 [ 0 : 미사용, 1 : 사용 ]')->default(0);
